...
c <- missmap(KingCounty) 
# Save an object to a file
saveRDS(missmap(KingCounty) , file = "MissingnessMap.rds")
...
saveRDS(BedVsPrice_Plot , file = "BedVsPrice_Plot.rds")
...
saveRDS(BedVsPrice_BoxPlot , file = "BedVsPrice_BoxPlot.rds")
...
saveRDS(BedVsPrice_Plot2 , file = "BedVsPrice_Plot2.rds")


#by far it is most common for homes to have 3 or 4 bedrooms
#the average price of a home increases betwen 1 and 8 bedrooms, but anything higher than 5 is really tough to call since there are very few homes with that many bedrooms
#after a certain point, the number of bedrooms does not really seem to be that useful, or in this case, does not seem to add value
...
saveRDS(BathVsPrice_BoxPlot , file = "BathVsPrice_BoxPlot.rds")
...
saveRDS(BathVsPrice_Plot , file = "BathVsPrice_Plot.rds")

#2.5 bathrooms is the most common, followed by 1
#the average price of a home rises with the number of bathrooms
...
saveRDS(ZipVsPrice_Plot , file = "ZipVsPrice_Plot.rds")
...
saveRDS(zipcode_map , file = "zipcode_map.rds")
...
#the most expensive zip code in this list is 98039, which is not that surprising when you realize that Bill Gates has a 66,000 square foot mansion located there - https://en.wikipedia.org/wiki/Bill_Gates%27s_house
#the most expensive zip code also has very few properties that sold in this time period, but other than that there are no obvious patterns between the number of properties sold per zip code and price
...
saveRDS(WaterfrontVsPrice_Plot , file = "WaterfrontVsPrice_Plot.rds")



#being waterfront triples the value
#waterfront is very rare
...
plot_Size
saveRDS(plot_Size , file = "plot_Size.rds")
...
SizeVsPrice_Plot <- ggarrange(plot_SqftPrice, plot_SqftFrequency, 
          ncol = 2, nrow = 1)
saveRDS(SizeVsPrice_Plot , file = "SizeVsPrice_Plot.rds")

#very large homes are rare and more expensive
...
install.packages("lwgeom")#libwgeom was throwing an error, trying this instead - package ‘libwgeom’ is not available (for R version 4.0.2)
install.packages("sf")
...
# #sample 30, just to make it easier to test the map
# set.seed(1234)
# small_king <- sample_n(KingCountyClean, size = 30)
# 
# #chaning to an sf object for better projection
# (small_king_sf <- st_as_sf(small_king, coords = c("long", "lat"), 
#                            crs = 4326, agr = "constant"))
# #view(small_king_sf) #just viewing to make sure it worked
# 
# newtestmap <- ggplot(data = world) +
#   geom_sf() +
#   geom_sf(data = counties) +
#   #geom_sf(data = small_sacramento2, size = 2, shape = 23, fill = "darkred") +
#   geom_sf(data = small_king_sf, aes(color = price)) +
#   coord_sf(xlim = c(-123.0, -121.0), ylim = c(47, 48), expand = FALSE)
# 
# newtestmap + scale_color_gradientn(colors = brewer.pal(3, "Dark2"))
# newtestmap + scale_color_gradientn(colors = brewer.pal(8, "RdYlGn"))
...
# #mapping price on the smaller set
# pricemap <- ggplot(data = world) +
#   geom_sf() +
#   #geom_sf(data = counties, fill = NA, color = gray(.5)) +
#   geom_sf(data = washington_cities, color = "#80b3ff", size = 6) +
#   geom_text_repel(data = washington_cities, aes(x = lng, y = lat, label = city), 
#                   fontface = "bold", size=6, 
#                   nudge_x = c(0.1, -.15, 0.3), 
#                   nudge_y = c(0.25, -0.1, -0.2),
#   ) +
#   geom_sf(data = small_king_sf, aes(color = price)) +
#   coord_sf(xlim = c(-123.0, -121.0), ylim = c(47, 48), expand = FALSE)
# pricemap + scale_color_gradientn(colors = brewer.pal(11, "RdYlGn"))
...
saveRDS(price_map , file = "price_map.rds")
...
saveRDS(MostExpensive_map , file = "MostExpensive_map.rds")
...
#####
# starting to build a prediction model
#####
...
sqft_reg_avgs <- trainNF %>%
  group_by(sqft2) %>%
  summarize(b_sqft = sum(price - mu)/(n()+lambda), n_i = n())
...
saveRDS(lambdas_qplot_sqft2 , file = "lambdas_qplot_sqft2.rds")
...
saveRDS(lambdas_qplot_zip , file = "lambdas_qplot_zip.rds")
...
# a <- a %>%
#   mutate(off = abs(pred-price),
#          percentoff = off/price)
# view(a)
...
saveRDS(neighbours_plot , file = "neighbours_plot.rds")
...
# #split some into a training/test set for evaluation later
# set.seed(1234)
# sacramento_split <- initial_split(sacramento, prop = 0.9, strata = price)
...
saveRDS(king_knn , file = "king_knn.rds")
...
saveRDS(knn_plot , file = "knn_plot.rds")
...
saveRDS(plot_pairs , file = "plot_pairs.rds")
...
library(GGally)
plot_pairs <- KingCountyClean %>% 
  select(price, sqft_living, bedrooms, grade) %>% 
  ggpairs()
plot_pairs
...
# training_rows <- KingCountyClean %>% 
#   select(price) %>% 
#   unlist() %>% # converts Class from a tibble to a vector
#   createDataPartition(p = 0.6, list = FALSE)
# 
# X_train <- KingCountyClean %>% 
#   select(sqft_living) %>% 
#   slice(training_rows) %>% 
#   data.frame()
# 
# Y_train <- KingCountyClean %>% 
#   select(price) %>% 
#   slice(training_rows) %>% 
#   unlist()
# 
# X_test <- KingCountyClean %>% 
#   select(sqft_living) %>% 
#   slice(-training_rows) %>% 
#   data.frame()
# 
# Y_test <- KingCountyClean %>% 
#   select(price) %>% 
#   slice(-training_rows) %>% 
#   unlist()
...
#   set_mode("regression")
# 
# lm_recipe <- recipe(price ~ sqft_living, data = sacramento_train)
# 
# lm_fit <- workflow() %>%
...
#   metrics(truth = price, estimate = .pred)
# lm_test_results  #rmse = $262,279
# 
# 
# lm_plot_final <- ggplot(sacramento_train, aes(x = sqft_living, y = price)) +
...
saveRDS(lm_plot_final , file = "lm_plot_final.rds")
...
#overall not bad, but does not perform as well on very expensive homes
